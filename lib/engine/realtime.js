...
function selectRealtimeWallboxes(wallboxPlans) {
    const candidates = [0, 1, 2].map(wb => ({
        ...
        outputActive: Boolean(getState(`${CFG.root}.Devices.Wallbox${wb}.OutputActive`)?.val),
...
function updateSlowTargets(requiredControlledW, wallboxPlans, heatingPlan, consumptionBudgetW = Infinity) {
    ...
    if (selected) {
        ...
        const productiveOutputActive = Boolean(
            getState(`${r}.Devices.Wallbox${candidate.wb}.OutputActive`)?.val);
        ...
        const allocationWallboxW = realtimeParallelActive && productiveOutputActive
    ...
    [0, 1, 2].forEach(wb => {
        if (wb === selectedWallbox || slowTargets.wallboxA[wb] <= 0) return;
