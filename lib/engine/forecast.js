...
function seriesValueAt(series, timestamp) {
    ...
    if (exact) return numericValue(exact.val);
    return null;
}

function configuredPriceSwitch(externalId, internalStateId, statusStateId) {
    if (!externalId) {
        write(statusStateId, 'intern');
        return Boolean(getState(internalStateId)?.val);
    }
    const external = readBooleanInput(externalId);
    if (external === null) {
        write(statusStateId, `extern ungueltig: ${externalId}; sicher AUS`);
        return false;
    }
    write(statusStateId, `extern: ${externalId}`);
    return external;
}

function buildPriceForecast(startTs) {
    const dynamicEnergyEnabled = configuredPriceSwitch(CFG.dp.dynamicEnergyPriceEnabled,
        `${CFG.root}.Config.DynamicEnergyPriceEnabled`,
        `${CFG.root}.Config.DynamicEnergyPriceSourceStatus`);
    const dynamicGridEnabled = configuredPriceSwitch(CFG.dp.dynamicGridFeeEnabled,
        `${CFG.root}.Config.DynamicGridFeeEnabled`,
        `${CFG.root}.Config.DynamicGridFeeSourceStatus`);
