...
function evaluateDhwSimulation() {
    ...
    const temperatures = CFG.dp.dhwTemps.map(id => freshDhwNumber(id, tankMaxAgeMs));
    const outletTemperature = freshDhwNumber(CFG.dp.myPvDhwOutletTemp, 2 * 60 * 1000);
    ...
    const valid = temperatures.length === 4 && temperatures.every(Number.isFinite)
        && Number.isFinite(outletTemperature);
    ...
    if (valid && (bottom >= stopTemperature || hottestTemperature >= topEmergencyStop)) dhwTemperatureLock = true;
    if (valid && bottom <= resumeTemperature && hottestTemperature < topEmergencyStop) dhwTemperatureLock = false;
