...
function evaluateDhwSimulation() {
    ...
    const available = Boolean(connectionState?.val);
    ...
    if (dhwTemperatureLock === null) {
        dhwTemperatureLock = Boolean(getState('javascript.0.ehz.hysterese')?.val);
    }
    ...
    if (!available) {
        ...
        reason = 'modbus.4 nicht verbunden';
    } else if (!valid) {
        temperaturePowerLimitW = 0;
        ...
        if (bottom >= 74) temperaturePowerLimitW = Math.min(3000, maxPower);
        else if (bottom >= 73) temperaturePowerLimitW = Math.min(4000, maxPower);
        else if (bottom >= 71) temperaturePowerLimitW = Math.min(6000, maxPower);
        else if (bottom >= 70) temperaturePowerLimitW = Math.min(7500, maxPower);
...
function simulateDhwTarget(requestedPowerW) {
    ...
    // Das Bestandsskript begrenzt die Aenderung auf 1 kW je 10 s. Der neue
    // Regler laeuft alle 2 s, entsprechend sind maximal 200 W je Zyklus erlaubt.
    const maximumStepW = 200;
