// Objects and native settings are fully initialized before schedules start.
write(`${CFG.root}.System.Version`, '0.17.0-alpha.19');
...
write(`${CFG.root}.System.ActiveScriptAudit_JSON`, JSON.stringify({
    ...
    correctedInAdapter: [
        ...
        'Ab 0.17.0-alpha.3: Eine erfolgreich uebernommene Wallbox ueberlebt ein kurzzeitiges Null-Soll waehrend der Reglerinitialisierung, ohne die Startverzoegerung neu zu aktivieren.',
        'Ab 0.17.0-alpha.4: Der Produktivausgang folgt der bestaetigten go-e-Phasenstellung und rechnet 6 A dreiphasig als 4.140 W. Nach Restart-Uebergabe beginnt die Mindestlaufzeit neu.',
        'Ab 0.17.0-alpha.5: Ein sinkendes Soll ersetzt einen noch offenen hoeheren Amperebefehl, ohne die Wallboxfreigabe zu entziehen.',
        'Ab 0.17.0-alpha.6: Eine neue Restart-Uebergabe startet auch im Echtzeitregler die produktive Mindestlaufzeit ohne OutputActive-Flanke neu.',
        'Ab 0.17.0-alpha.7: Die Restart-Uebergabe endet erst nach durchgehend stabilen frischen EMS-Daten; Initialisierungs-Nullwerte bleiben geschuetzt.',
        'Ab 0.17.0-alpha.8: Der 50/50-Schalter waehlt nur die Verteilung; AUS laesst die Wallbox mit Vorrang weiterlaufen.',
        'Ab 0.17.0-alpha.9: Eine begonnene Wallbox-Startsequenz bleibt bis zur bestaetigten Ladefreigabe gegen weiche Sollwertspruenge geschuetzt.',
        'Ab 0.17.0-alpha.10: LastStopReason und LastStopAt halten den letzten produktiven Abschaltgrund dauerhaft fest.',
        'Ab 0.17.0-alpha.11: Die Restart-Uebergabe wartet auf einen nach diesem Neustart frisch berechneten Fahrplan und Reglerlauf.',
        'Ab 0.17.0-alpha.12: Produktive Abschaltungen werden einmalig protokolliert und nennen den konkret fehlenden Messwert.',
        'Ab 0.17.0-alpha.13: go-e-Messwerte haben eine konfigurierbare Frischefrist von standardmaessig 30 s statt einer zu knappen 15-s-Grenze.',
        'Ab 0.17.0-alpha.14: Kurze Leistungsdellen reduzieren eine laufende Wallbox auf 6 A; eine eigene konfigurierbare Ausschaltverzoegerung startet bei jeder neuen Unterschreitung.',
        'Ab 0.17.0-alpha.15: Native Freigaben vor Reglerstart; harte Stopps vor Restart-Wartepfaden; geordnete Schreibbestaetigungen; produktive Budgetauswahl ohne simulierte Lasten; schnelle EHZ-Abregelung und Netzueberschuss-Nachfuehrung.',
        'Ab 0.17.0-alpha.16: Eigene passive Debug-Objekte mit Klartext, Ereignishistorie und Leistungsverlauf; keine Aenderung der Stellregeln.',
        'Ab 0.17.0-alpha.17: Ein zentraler Feinregler fuer Sun-Energy-Speicher oder EHZ, zwei getrennte my-PV-Heizkreise mit Kuehlsperre, begrenztes Preisheizen und passive WP-Empfehlung; neue Ausgaenge separat freizugeben.',
        'Ab 0.17.0-alpha.18: Wallboxbegrenzung gibt nur echte PV frei; verworfene Netzladeleistung erhoeht keinen EHZ-/Speicherfahrplan. Plan.Allocation_48h_JSON zeigt PV-/Netzanteile und Warmwassergrund je Intervall.',
        'Ab 0.17.0-alpha.19: Debug.Shadow berechnet bei Master AUS die gemeinsamen Produktiventscheidungen in isolierter Umgebung; eigene Soll-/Ist- und Statusreihen werden mit 24-h-Aufbewahrung in SQL eingerichtet.'
    ]
}));
...
schedule('*/10 * * * * *', updateHeatingSimulation);
schedule('*/10 * * * * *', updateHeatPumpAdvice);
schedule('*/2 * * * * *', realtimeControl);
schedule('*/2 * * * * *', updateBatteryProductionOutput);
schedule('*/5 * * * * *', updateDhwProductionOutput);
schedule('*/5 * * * * *', updateHeatingProductionOutput);
...
log('EMS V0.17.0-alpha.19 mit isoliertem Schattenbetrieb und 24-h-SQL-Diagnose gestartet', 'info');
