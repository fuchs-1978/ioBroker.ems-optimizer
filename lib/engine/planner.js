...
function quantizePlannedWallbox(requestedW, vehicle) {
    ...
    return amps >= 6 ? amps * voltage * phases : 0;
}
...
function buildDevicePlan(data) {
    ...
    for (let i = 0; i < data.pv.length; i++) {
        ...
        } else if (activeWallbox !== null && (remainingPvW >= activeVehicle.minimumPowerW || deadlineCharge)) {
            wbW[activeWallbox] = Math.min(wallboxCapW,
                deadlineCharge ? activeVehicle.maximumPowerW : remainingPvW);
            if (!deadlineCharge && wbW[activeWallbox] < activeVehicle.minimumPowerW) wbW[activeWallbox] = 0;
        ...
        if (activeWallbox !== null && wbW[activeWallbox] > 0) {
            ...
            wbW[activeWallbox] = quantizePlannedWallbox(beforeQuantizationW, activeVehicle);
