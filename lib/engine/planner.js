...
function quantizePlannedWallbox(requestedW, vehicle, phases = 1) {
    ...
    const maximumA = Math.max(minimumA, Math.min(vehicleMaximumA,
...
function buildDevicePlan(data) {
    ...
    for (let i = 0; i < data.pv.length; i++) {
        ...
        for (const wb of eligibleWallboxes) {
            if (wb === activeWallbox || wallboxRemainingKWh[wb] <= 0) continue;
            const vehicle = wallboxStatus[wb];
            ...
            const quantizedW = quantizePlannedWallbox(requestedW, vehicle, phases);
            if (quantizedW < minimumPowerW) continue;
            wbW[wb] = quantizedW;
