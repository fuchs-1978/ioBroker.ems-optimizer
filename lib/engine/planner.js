...
function recommendedPlanPhases(vehicle, remainingPvW, remainingGridKWh, timestamp) {
    ...
    const hoursRemaining = Math.max(0.25, (vehicle.departureTimestamp - timestamp) / 3600000);
    const averageRequiredW = remainingGridKWh / hoursRemaining * 1000;
...
function quantizePlannedWallbox(requestedW, vehicle, phases = 1, partialSlot = false) {
    ...
    // Actual output still uses at least minimumA for a shorter duration.
    if (partialSlot && maximumA >= minimumA && requestedW < minimumA * voltage * phases)
        return requestedW;
    const amps = Math.min(maximumA, Math.floor(requestedW / (voltage * phases)));
...
function buildDevicePlan(data) {
    ...
    for (let i = 0; i < data.pv.length; i++) {
        ...
        const slotVehicles = wallboxStatus.map((v, wb) => plannedVehicleAtSoc(v, wallboxRemainingKWh[wb]));
        const slotOrder = [...eligibleWallboxes].sort((a, b) =>
            Number(slotVehicles[b].belowMinimum) - Number(slotVehicles[a].belowMinimum)
            || Number(slotVehicles[b].selectedPriority) - Number(slotVehicles[a].selectedPriority)
            || slotVehicles[b].priority - slotVehicles[a].priority);
        const activeWallbox = slotOrder.find(wb => wallboxRemainingKWh[wb] > 0
        ...
        const deadlineCharge = activeVehicle !== null
            && (activeVehicle.belowMinimum || (activeVehicle.deadlineEnabled && timestamp >= activeVehicle.latestStartTimestamp))
        ...
        if (forcedDhw) {
            ...
            const minNeedW = activeVehicle.belowMinimum
                ? (activeVehicle.minimumSocPct - activeVehicle.socPct) / 100 * activeVehicle.capacityKWh
                    / vehicleEfficiency * 4000 : wallboxCapW;
            ...
            if (!deadlineCharge && wbW[activeWallbox] < activeMinimumPowerW) wbW[activeWallbox] = 0;
            remainingPvW = Math.max(0, remainingPvW - wbW[activeWallbox]);
        }

        if (activeWallbox !== null && wbW[activeWallbox] > 0) {
            const beforeQuantizationW = wbW[activeWallbox];
            wbW[activeWallbox] = quantizePlannedWallbox(beforeQuantizationW, activeVehicle, activePhases,
                deadlineCharge || energyLimitedW <= remainingPvW + beforeQuantizationW);
            remainingPvW += Math.max(0, beforeQuantizationW - wbW[activeWallbox]);
        }
        ...
        // Ist das priorisierte Fahrzeug versorgt und bleibt noch mindestens
        // die 6-A-Mindestleistung uebrig, duerfen weitere Fahrzeuge parallel
        // laden. Reihenfolge bleibt die berechnete Fahrzeugprioritaet.
        for (const wb of slotOrder) {
            if (wb === activeWallbox || wallboxRemainingKWh[wb] <= 0) continue;
            const vehicle = slotVehicles[wb];
            const phases = recommendedPlanPhases(vehicle, remainingPvW,
                wallboxRemainingKWh[wb], timestamp);
            wbPhases[wb] = phases;
            const minimumA = phases === 3 ? vehicle.minCurrent3pA : vehicle.minCurrent1pA;
            const minimumPowerW = minimumA * 230 * phases;
            if (timestamp >= vehicle.departureTimestamp || remainingPvW < minimumPowerW) continue;
            const energyCapW = Number.isFinite(wallboxRemainingKWh[wb])
                ? wallboxRemainingKWh[wb] / 0.25 / vehicleEfficiency * 1000
                : vehicle.maximumPowerW;
            const requestedW = Math.min(vehicle.maximumPowerW, energyCapW, remainingPvW);
            const quantizedW = quantizePlannedWallbox(requestedW, vehicle, phases, energyCapW <= remainingPvW);
            if (quantizedW <= 0) continue;
            wbW[wb] = quantizedW;
            remainingPvW = Math.max(0, remainingPvW - quantizedW);
            if (Number.isFinite(wallboxRemainingKWh[wb])) {
                wallboxRemainingKWh[wb] = Math.max(0,
                    wallboxRemainingKWh[wb] - quantizedW * 0.25 / 1000 * vehicleEfficiency);
            }
        }
