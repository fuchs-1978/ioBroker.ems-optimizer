...
function dhwCombinedProductionState() {
    ...
    const allowed = selected.length === 1
        && Boolean(getState(`${CFG.root}.Config.DHWParallelDistributionEnabled`)?.val)
        && nativeConfig.combinedProductionArmed === true
        && nativeConfig[`wb${selected[0]}ProductionArmed`] === true
        && release === true;
    let reason = '';
    if (selected.length !== 1) reason = 'genau eine Wallbox muss fuer Kombination freigegeben sein';
    else if (nativeConfig.combinedProductionArmed !== true) reason = 'Kombinationstest nicht bestaetigt';
    else if (nativeConfig[`wb${selected[0]}ProductionArmed`] !== true) reason = 'Wallbox-Einzeltest nicht bestaetigt';
...
function dhwOutputStatus(active, commandW, status, wrote = false) {
    ...
    write(`${CFG.root}.Control.Mode`, active && wallboxOwned ? 'WALLBOX_DHW_COMBINED'
