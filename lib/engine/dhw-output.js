...
function phaseLimitedDhwPower(requestedW) {
    const freeIds = CFG.dp.myPvDhwHaFreeCurrentA || [];
    const outputIds = CFG.dp.myPvDhwOutputW || [];
    if (freeIds.length !== 3 || outputIds.length !== 3) return 0;
    const allowed = [0, 1, 2].map(index => {
        const freeA = validDhwOutputNumber(freeIds[index]);
        const actualW = validDhwOutputNumber(outputIds[index]);
        if (freeA === null || actualW === null) return null;
...
function updateDhwProductionOutput() {
    ...
    const temperatures = CFG.dp.dhwTemps.map(id => validDhwOutputNumber(id));
