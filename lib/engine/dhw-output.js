...
function dhwOutputStatus(active, commandW, status, wrote = false) {
    ...
    write(`${CFG.root}.Control.Mode`, active ? 'DHW_PRODUCTION'
        : wallboxOwned ? 'WALLBOX_SINGLE_TEST' : 'SIMULATION');
}
...
function updateDhwProductionOutput() {
    ...
    if (simulatedW <= 0) {
        ...
        if (actuatorSettled || timedOut) {
            desiredW = Math.min(releaseCapW, actualW - gridErrorW);
    ...
    desiredW = Math.max(0, Math.min(releaseCapW, desiredW));
    const lowerW = Math.max(0, dhwLastCommandW - stepW);
    const upperW = Math.min(releaseCapW, dhwLastCommandW + stepW);
    const commandW = simulatedW <= 0 ? 0 : (stepW === 0 ? dhwLastCommandW
        : Math.round(Math.max(lowerW, Math.min(upperW, desiredW))));
    ...
    dhwOutputStatus(true, commandW,
        `PRODUKTIV: ${commandW} W; ${reason}; Grenze ${commissioningMaxW} W`, true);
}
