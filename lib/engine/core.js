 ...
 *
 * HISTORIE
 * 0.17.0-alpha.19 - isolierter Schattenbetrieb mit Produktivregellogik und 24-h-SQL-Diagnose.
 * 0.17.0-alpha.18 - korrigierte PV-Restbilanz nach Wallboxbegrenzung und Fahrplan-Quelldiagnose.
 * 0.17.0-alpha.17 - koordinierter Speicher, zwei Heizkreise und passive WP-Empfehlung.
 * 0.17.0-alpha.16 - passive Debug-Snapshots, Ereignisse und begrenzter Leistungsverlauf.
 * 0.17.0-alpha.15 - geprüfte Start-/Stopp-Übergaben, produktive Budgetauswahl und EHZ-Nachführung.
 * 0.17.0-alpha.14 - eigene Ausschaltverzoegerung fuer kurze Leistungsdellen.
 * 0.17.0-alpha.13 - praxisgerechte konfigurierbare Frischefrist fuer go-e-Messwerte.
 * 0.17.0-alpha.12 - eindeutige einmalige Abschaltdiagnose mit konkretem Messwert.
 * 0.17.0-alpha.11 - Restart-Handoff wartet auf nach dem Neustart frisch berechneten Fahrplan.
...
const CFG = {
    ...
    dp: {
        ...
        heatPumpBufferTemp: '__DP_HEAT_PUMP_BUFFER_TEMP__',
        heatPumpDhwTemp: '__DP_HEAT_PUMP_DHW_TEMP__',
        batterySoc: '__DP_BATTERY_SOC__',
        batteryPower: '__DP_BATTERY_POWER__',
        batteryAcPower: '__DP_BATTERY_AC_POWER__',
...
function createStates() {
    const r = CFG.root;
    stateDef(`${r}.System.Version`, '0.17.0-alpha.19', 'string', 'text', '', 'EMS-Version');
    ...
    configDef(`${r}.Config.WallboxStopDelay_s`, 120, 'number', 'value.interval', 's', 'Ausschaltverzoegerung bei voruebergehend zu wenig Ladeleistung');
    ...
    configDef(`${r}.Config.WallboxMeasurementMaxAge_s`, 30, 'number', 'value.interval', 's', 'Maximales Alter der go-e-Messwerte');
    ...
    stateDef(`${r}.Plan.Allocation_48h_JSON`, '[]', 'string', 'json', '', 'PV- und Netzanteile je Fahrplanintervall');
    stateDef(`${r}.Plan.AllocationSource`, '', 'string', 'text', '', 'Grundlastquelle der Fahrplanbilanz');
    ...
    stateDef(`${r}.Devices.MyPV_DHW.OutputOwned`, false, 'boolean', 'indicator', '', 'EHZ-Ausgang vom EMS uebernommen; bei AUS bis Abschluss des Abschaltbefehls');
    stateDef(`${r}.Devices.MyPV_DHW.OutputSetpointId`, '', 'string', 'text', '', 'Tatsaechlich uebernommener EHZ-Ausgang');
    stateDef(`${r}.Devices.MyPV_DHW.DriverReady`, false, 'boolean', 'indicator', '', 'EHZ-Ausgangsmetadaten geprueft');
    stateDef(`${r}.Devices.MyPV_DHW.DriverStatus`, 'Nicht geprueft', 'string', 'text', '', 'Validierung des EHZ-Ausgangs');
    for (const name of ['OutputReservedPower_W', 'OutputReservedPhase1_W', 'OutputReservedPhase2_W', 'OutputReservedPhase3_W'])
        stateDef(`${r}.Devices.MyPV_DHW.${name}`, 0, 'number', 'value.power', 'W', 'Noch nicht sicher freigewordene Leistungsreserve');
    stateDef(`${r}.Devices.MyPV_DHW.OutputReservationPending`, false, 'boolean', 'indicator', '', 'Frueherer Stellauftrag noch nicht abschliessend geklaert');
    stateDef(`${r}.Devices.MyPV_DHW.OutputReservationState_JSON`, '{}', 'string', 'json', '', 'Persistierter Nachweis ausstehender Stellwirkung');
    stateDef(`${r}.Devices.MyPV_DHW.OutputReservationStatus`, '', 'string', 'text', '', 'Begruendung ausstehender Leistungsreserve');
    configDef(`${r}.Devices.MyPV_DHW.ConfirmPhysicalStop`, false, 'boolean', 'button', '', 'Nur nach unabhaengig bestaetigtem Geraetestopp und ausgeschlossenen alten Stellauftraegen bestaetigen');
...
// Number(null), Number('') and Number(false) are all zero. These are not
// measurements and must not make a missing sensor look like a valid reading.
...
function readNumber(id, fallback = 0) {
    try {
        ...
        const s = getState(id);
        // Legacy release objects are deliberately consumed numerically by the
        // vehicle manager. Keep explicit booleans there, but reject empties.
        const value = s?.val;
...
function readFreshNumber(id, invalid, required = true) {
    try {
        ...
        if (timestamp === null || timestamp <= 0 || ageMs < -5000 || ageMs > CFG.dataMaxAgeMs) {
