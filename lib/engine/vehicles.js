...
function plannedVehicleAtSoc(vehicle, remainingBatteryKWh) {
    ...
    return {...vehicle, socPct: soc,
        belowMinimum: vehicle.socValid && soc < vehicle.minimumSocPct,
        maxCurrent1pA: taperCurrentLimit(vehicle.index, soc, vehicle.targetSocPct, vehicle.baseMaxCurrent1pA),
        maxCurrent3pA: taperCurrentLimit(vehicle.index, soc, vehicle.targetSocPct, vehicle.baseMaxCurrent3pA)};
}
...
function vehicleState(wb) {
    ...
    return {
        ...
        baseMaxCurrent1pA: readNumber(`${r}.MaxCurrent1P_A`, 32),
        maxCurrent1pA: taperCurrentLimit(wb, readNumber(`${r}.SoC_pct`, 0),
            readNumber(`${r}.TargetSoC_pct`, 100), readNumber(`${r}.MaxCurrent1P_A`, 32)),
        minCurrent3pA: readNumber(`${r}.MinCurrent3P_A`, wb === 0 ? 0 : 6),
        baseMaxCurrent3pA: readNumber(`${r}.MaxCurrent3P_A`, 16),
        maxCurrent3pA: taperCurrentLimit(wb, readNumber(`${r}.SoC_pct`, 0),
            readNumber(`${r}.TargetSoC_pct`, 100), readNumber(`${r}.MaxCurrent3P_A`, 16)),
...
function updateVehicles() {
    ...
    const selectedPriorityIndex = configuredPriority === -2
    ...
    for (let wb = 0; wb < 3; wb++) {
        ...
        const deadlineReached = Boolean(vehicleSetting(wb, 'DeadlineEnabled', false))
