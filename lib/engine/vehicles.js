...
function updateVehicles() {
    ...
    for (let wb = 0; wb < 3; wb++) {
        ...
        const socNumeric = typeof socState?.val === 'number'
            || (typeof socState?.val === 'string' && socState.val.trim() !== '');
        const socTimestamp = Number(socState?.ts);
        const socValid = socNumeric && socState?.ack === true && !(socState?.q)
            && Number.isFinite(socNumber) && socNumber >= 0 && socNumber <= 100
            && Number.isFinite(socTimestamp) && socTimestamp > 0 && socTimestamp <= now + 1000
            && now - socTimestamp <= 2 * 60 * 60 * 1000;
