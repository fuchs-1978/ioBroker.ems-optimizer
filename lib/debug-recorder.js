...
// Identical byte budget for generated history exports and restored history.
const MAX_RESTORE_BYTES = 2 * 1024 * 1024;
const TEXT_LIMIT = 600;
const COORDINATION_FIELDS = ['FineRegulator', 'HeaterBudgetMode', 'AllowHeaterGridImport',
    'CoordinationActive', 'CoordinationStatus', 'UncontrolledGridPower_W', 'AvailablePVPower_W',
    'BatteryPVReserve_W', 'HeaterPVAllocation_W', 'HeaterCheapGridAllocation_W',
    'ProtectedGridImport_W', 'GrossConsumptionTarget_W', 'CheapEnergyAllowed',
    'ThermalPriceValid', 'CurrentTotalPrice_ct_kWh', 'ThermalPriceSource', 'ThermalPricePolicyStatus'];
const BATTERY_CONTEXT_FIELDS = ['actual_W', 'target_W', 'commandInternal_W', 'commandGS_W',
    'soc_pct', 'active', 'owned', 'status', 'fault', 'reservedCharge_W', 'unobservedCommand_W'];
const HEATER_RESERVATION_FIELDS = ['reservedPower_W', 'reservedPhase1_W', 'reservedPhase2_W',
    'reservedPhase3_W', 'reservationPending', 'reservationStatus'];
const HEATING_CONTEXT_FIELDS = ['actual_W', 'target_W', 'command_W', 'active', 'owned',
    'status', 'fault', 'coolingBlocked', 'inhibited', ...HEATER_RESERVATION_FIELDS];
const HEAT_PUMP_CONTEXT_FIELDS = ['mode', 'valid', 'reason', 'holdRemaining_s', 'coolingActive'];
const DISCRETE = [
    ...
    ...['Present', 'ControlEnabled', 'Release', 'OutputActive', 'OutputOwned', 'TemperatureLock', 'OutputReservationPending']
        .map(key => `Devices.MyPV_DHW.${key}`),
    ...['Present', 'ControlEnabled', 'DriverReady', 'RegulationAvailable', 'CanCharge',
        'CanDischarge', 'OutputOwned', 'OutputActive', 'Fault'].map(key => `Devices.Battery.${key}`),
    ...['Present', 'ControlEnabled', 'DriverReady', 'Release', 'OutputActive', 'OutputOwned',
        'OutputFault', 'LastStopAt', 'CoolingBlocked', 'Inhibited', 'TemperatureValid', 'OutputReservationPending']
        .map(key => `Devices.MyPV_Heating.${key}`),
    ...['Present', 'ControlEnabled', 'RequestedMode', 'AdviceEnabled', 'AdviceValid',
        'CoolingActive', 'CoolingDataValid'].map(key => `Devices.HeatPump.${key}`),
    ...['FineRegulator', 'HeaterBudgetMode', 'AllowHeaterGridImport', 'CoordinationActive',
        'CheapEnergyAllowed', 'ThermalPriceValid'].map(key => `Control.${key}`)
];
...
const REASONS = ['Control.Status', 'Control.GridOperatorStatus', 'History.Status',
    ...
    'Devices.Battery.OutputStatus', 'Devices.Battery.DriverStatus',
    'Devices.MyPV_Heating.OutputStatus', 'Devices.MyPV_Heating.Status',
    'Devices.MyPV_Heating.DriverStatus', 'Devices.HeatPump.AdviceReason',
    'Control.CoordinationStatus', 'Control.ThermalPricePolicyStatus',
    'Devices.MyPV_DHW.OutputReservationStatus', 'Devices.MyPV_Heating.OutputReservationStatus'];
...
function heaterReservationContext(source) {
    return {reservedPower_W: source.OutputReservedPower_W,
        reservedPhase1_W: source.OutputReservedPhase1_W, reservedPhase2_W: source.OutputReservedPhase2_W,
        reservedPhase3_W: source.OutputReservedPhase3_W, reservationPending: source.OutputReservationPending,
        reservationStatus: source.OutputReservationStatus};
}
...
class DebugRecorder {
    ...
    async initialize() {
        try {
            const definitions = {
                ...
                'Battery.Summary': ['', 'text', 'Kurzdiagnose Speicher mit eindeutigen Leistungsvorzeichen'],
                'Heating.Summary': ['', 'text', 'Kurzdiagnose Heizpuffer-EHZ'],
                'HeatPump.Summary': ['', 'text', 'WP-Betriebsartempfehlung ohne externe Schaltung'],
                'Coordination.Summary': ['', 'text', 'Aktiver Feinregler und gemeinsame Leistungsbudgets']
            };
    ...
    restore(key, limit, transform) {
        ...
        return this.guard(() => {
            ...
            // The complete payload is already byte-bounded. The transform
            // copies only fixed keys and bounded strings, including legacy
            // entries; avoid rejecting valid multibyte/escaped own messages.
            return parsed.slice(-limit).map(transform).filter(Boolean)
    ...
    heartbeat(id, now, maxAgeMs, allowISO = false, requireFreshState = true) {
        const result = this.measurement(id, now, requireFreshState ? maxAgeMs : Infinity);
        const numeric = number(result.val);
        const valueTs = numeric === null && allowISO && typeof result.val === 'string'
            ? Date.parse(result.val) : numeric;
        const stateValid = result.ts > 0 && result.ageMs >= 0 && result.ack === true && !result.q
            && (!requireFreshState || result.ageMs <= maxAgeMs);
        const valueValid = Number.isFinite(valueTs) && valueTs > 0 && valueTs <= now
            && now - valueTs <= maxAgeMs;
        return {...result, valueTimestamp: Number.isFinite(valueTs) ? valueTs : null,
            valueAgeMs: Number.isFinite(valueTs) ? now - valueTs : null,
            valid: stateValid && valueValid,
            reason: stateValid && valueValid ? 'OK' : 'Heartbeat fehlt/ungueltig/veraltet'};
    }
    ...
    snapshot() {
        ...
        const ehz = {...fields('Devices.MyPV_DHW', ['Present', 'ControlEnabled', 'Release', 'OutputActive',
            ...
            'OutputReservedPhase3_W', 'OutputReservationPending', 'OutputReservationStatus']),
        ...
        const setting = (key, native, fallback) => number(this.own(`Config.${key}`))
            ?? number(config[native]) ?? fallback;
        const batteryAgeMs = setting('BatteryMeasurementMaxAge_s', 'batteryMeasurementMaxAgeS', 30) * 1000;
        const batterySocAgeMs = setting('BatterySoCMaxAge_s', 'batterySoCMaxAgeS', 300) * 1000;
        const batteryHeartbeat = this.heartbeat(config.batteryHeartbeatId || mapping.DP_BATTERY_HEARTBEAT,
            now, batteryAgeMs, true, false);
        const batteryHead = String(config.batterySetpointId || '')
            .match(/^(sunenergyxt500\.\d+)\.(heads\.\d+)\.control\.GS$/);
        const batterySource = (id, maximumAgeMs) => {
            const ownHead = source => batteryHead && typeof source === 'string'
                && source.startsWith(`${batteryHead[1]}.${batteryHead[2]}.`);
            const coherent = Boolean(batteryHead && (ownHead(id)
                || typeof id === 'string' && id.startsWith(`${batteryHead[1]}.total.`)
                    && this.own('Devices.Battery.SingleHeadVerified') === true)
                && config.batteryHeartbeatId === `${batteryHead[1]}.info.lastUpdate`
                && [config.batteryOnlineId, config.batteryManualModeId, config.batteryLocalModeId].every(ownHead));
            const result = this.measurement(id, now,
                coherent && batteryHeartbeat.valid ? Infinity : maximumAgeMs);
            return {...result, driverBound: coherent, rawAgeLimitMs: maximumAgeMs,
                validatedByHeartbeat: coherent && batteryHeartbeat.valid && result.valid,
                freshnessSource: coherent && batteryHeartbeat.valid ? 'same-driver heartbeat' : 'state timestamp'};
        };
        const batteryMeasurements = {
            power: batterySource(config.batteryAcPowerId || mapping.DP_BATTERY_AC_POWER, batteryAgeMs),
            dcPower: batterySource(config.batteryPowerId || mapping.DP_BATTERY_POWER, batteryAgeMs),
            soc: batterySource(config.batterySocId || mapping.DP_BATTERY_SOC, batterySocAgeMs),
            heartbeat: batteryHeartbeat,
            online: this.measurement(config.batteryOnlineId || mapping.DP_BATTERY_ONLINE, now, Infinity, false, true),
            manualMode: this.measurement(config.batteryManualModeId || mapping.DP_BATTERY_MANUAL_MODE, now, Infinity, false, true),
            localMode: this.measurement(config.batteryLocalModeId || mapping.DP_BATTERY_LOCAL_MODE, now, Infinity, false, true),
            fault: this.measurement(config.batteryFaultId || mapping.DP_BATTERY_FAULT, now, Infinity, false, true),
            temperature: this.measurement(config.batteryTemperatureId || mapping.DP_BATTERY_TEMPERATURE, now, batteryAgeMs)
        };
        const acPowerId = config.batteryAcPowerId || mapping.DP_BATTERY_AC_POWER;
        const acSourceMatches = Boolean(batteryHead && batteryMeasurements.power.driverBound
            && [ `${batteryHead[1]}.${batteryHead[2]}.grid.GP`, `${batteryHead[1]}.total.gridPower` ].includes(acPowerId));
        batteryMeasurements.power.unit = 'W';
        batteryMeasurements.power.kind = 'AC GP: positive discharge';
        batteryMeasurements.power.acceptedAsGSFeedback = acSourceMatches;
        batteryMeasurements.dcPower.unit = 'W';
        batteryMeasurements.dcPower.kind = 'DC BP: history/planning only';
        const battery = {...fields('Devices.Battery', ['Present', 'ControlEnabled', 'DriverReady', 'DriverStatus',
            'SingleHeadVerified', 'RegulationAvailable', 'CanCharge', 'CanDischarge', 'RequestedPower_W',
            'OutputCommand_W', 'OutputCommandInternal_W', 'ActualPower_W', 'OutputOwned', 'OutputActive',
            'OutputStatus', 'Fault', 'ActuatorSettled', 'ActuatorDifference_W', 'CommandAge_s', 'EffectiveStep_W',
            'OutputReservedCharge_W', 'OutputUnobservedCommand_W', 'OutputUnobservedCommandSince']),
        productionArmed: config.batteryProductionArmed === true,
        signConvention: {actualAndInternal: 'positive = charge; negative = discharge',
            requestedAndGS: 'positive = discharge; negative = charge',
            measuredACPowerMultiplier: -1, dcPower: 'separate BP telemetry; not GS feedback'},
        target_W: this.own('Control.Targets.Battery_W'),
        actual_W: batteryMeasurements.power.valid && acSourceMatches
            ? -number(batteryMeasurements.power.val) : null,
        soc_pct: batteryMeasurements.soc.valid ? number(batteryMeasurements.soc.val) : null,
        minimumSoC_pct: this.own('Config.BatteryMinSoC_pct'), maximumSoC_pct: this.own('Config.BatteryMaxSoC_pct'),
        fineReserve_W: this.own('Config.BatteryFineReserve_W'), measurements: batteryMeasurements};
        const heatingAgeMs = setting('HeatingTemperatureMaxAge_s', 'heatingTemperatureMaxAgeS', 3600) * 1000;
        const heatingOutputAgeMs = setting('HeatingOutputMaxAge_s', 'heatingOutputMaxAgeS', 120) * 1000;
        const coolingAgeMs = setting('HeatingCoolingMaxAge_s', 'heatingCoolingMaxAgeS', 120) * 1000;
        const coolingHeartbeatId = config.heatingCoolingHeartbeatId || mapping.DP_HEAT_COOLING_HEARTBEAT;
        const coolingHeartbeat = this.heartbeat(coolingHeartbeatId, now, coolingAgeMs);
        const cooling = this.measurement(config.heatingCoolingActiveId || mapping.DP_HEAT_COOLING_ACTIVE,
            now, coolingHeartbeatId && coolingHeartbeat.valid ? Infinity : coolingAgeMs, false, true);
        if (coolingHeartbeatId && !coolingHeartbeat.valid) {
            cooling.valid = false;
            cooling.reason = 'Konfigurierter Kuehlstatus-Heartbeat fehlt/ungueltig/veraltet';
        }
        cooling.validatedByHeartbeat = Boolean(coolingHeartbeatId && coolingHeartbeat.valid && cooling.valid);
        const heatingMeasurements = {
            outputs: [1, 2, 3].map(i => this.measurement(config[`heatingOutput${i}Id`]
                || mapping[`DP_HEAT_OUTPUT${i}`], now, heatingOutputAgeMs)),
            temperature: this.measurement(config.heatingTempId || mapping.DP_HEAT_TEMP, now, heatingAgeMs),
            outletTemperature: this.measurement(config.heatingOutletTempId || mapping.DP_HEAT_OUTLET_TEMP, now, heatingAgeMs),
            connection: this.measurement(config.heatingConnectionId || mapping.DP_HEAT_CONNECTION, now, Infinity, false, true),
            cooling, coolingHeartbeat
        };
        const heating = {...fields('Devices.MyPV_Heating', ['Present', 'ControlEnabled', 'Available', 'Release',
            'DriverReady', 'DriverStatus', 'TemperaturePowerLimit_W', 'Temperature_C', 'TemperatureValid',
            'CoolingDataValid', 'CoolingActive', 'CoolingBlocked', 'Inhibited', 'RemainingCapacity_kWh',
            'ActualPower_W', 'Status', 'OutputOwned', 'OutputActive', 'OutputCommand_W', 'OutputLastWrite',
            'OutputStatus', 'OutputFault', 'LastStopReason', 'LastStopAt', 'ActuatorSettled',
            'OutputReservedPower_W', 'OutputReservedPhase1_W', 'OutputReservedPhase2_W',
            'OutputReservedPhase3_W', 'OutputReservationPending', 'OutputReservationStatus']),
        productionArmed: config.heatingProductionArmed === true,
        target_W: this.own('Control.Targets.MyPV_Heating_W'),
        actual_W: heatingMeasurements.outputs.every(m => m.valid && number(m.val) >= 0)
            ? heatingMeasurements.outputs.reduce((sum, m) => sum + number(m.val), 0) : null,
        measurements: heatingMeasurements};
        const heatPumpAgeMs = setting('HeatPumpTemperatureMaxAge_s', 'heatPumpTemperatureMaxAgeS', 3600) * 1000;
        const heatPump = {...fields('Devices.HeatPump', ['Present', 'ControlEnabled', 'RequestedMode',
            'RequestedModeValue', 'AdviceValid', 'AdviceEnabled', 'HeatRoomAvailable', 'HeatingRoomAvailable',
            'DHWRoomAvailable', 'CoolingActive', 'CoolingDataValid', 'PVSurplus_W', 'HoldRemaining_s',
            'LastModeChange', 'AdviceLastUpdate', 'AdviceReason', 'OutputOwned', 'OutputActive']),
        adviceOnly: true, measurements: {
            bufferTemperature: this.measurement(config.heatPumpBufferTemperatureId || mapping.DP_HEAT_PUMP_BUFFER_TEMP
                || config.heatingTempId || mapping.DP_HEAT_TEMP, now, heatPumpAgeMs),
            dhwTemperature: this.measurement(config.heatPumpDhwTemperatureId || mapping.DP_HEAT_PUMP_DHW_TEMP, now, heatPumpAgeMs),
            power: this.measurement(config.heatPumpPowerId || mapping.DP_HEAT_PUMP_POWER, now),
            cooling, coolingHeartbeat
        }};
        const coordination = fields('Control', COORDINATION_FIELDS);
        if (coordination.ThermalPriceValid !== true) coordination.CurrentTotalPrice_ct_kWh = null;
        ...
        return {schemaVersion: 2, ...stamp(now), session: this.session,
            ...
            wallboxes, ehz, battery, heating, heatPump, coordination,
            measurements, houseConnection: houseConnectionSettings(config)};
    }
    ...
    context(snapshot) {
        return {selectedWallbox: selected(snapshot.control.SelectedWallbox),
            ...
            ehz: {actual_W: snapshot.ehz.actual_W, target_W: snapshot.ehz.target_W,
                ...
                ...heaterReservationContext(snapshot.ehz)},
            battery: {actual_W: snapshot.battery.actual_W, target_W: snapshot.battery.target_W,
                commandInternal_W: snapshot.battery.OutputCommandInternal_W,
                commandGS_W: snapshot.battery.OutputCommand_W, soc_pct: snapshot.battery.soc_pct,
                active: snapshot.battery.OutputActive, owned: snapshot.battery.OutputOwned,
                status: snapshot.battery.OutputStatus, fault: snapshot.battery.Fault,
                reservedCharge_W: snapshot.battery.OutputReservedCharge_W,
                unobservedCommand_W: snapshot.battery.OutputUnobservedCommand_W},
            heating: {actual_W: snapshot.heating.actual_W, target_W: snapshot.heating.target_W,
                command_W: snapshot.heating.OutputCommand_W, active: snapshot.heating.OutputActive,
                owned: snapshot.heating.OutputOwned, status: snapshot.heating.OutputStatus,
                fault: snapshot.heating.OutputFault, coolingBlocked: snapshot.heating.CoolingBlocked,
                inhibited: snapshot.heating.Inhibited, ...heaterReservationContext(snapshot.heating)},
            heatPump: {mode: snapshot.heatPump.RequestedMode, valid: snapshot.heatPump.AdviceValid,
                reason: snapshot.heatPump.AdviceReason, holdRemaining_s: snapshot.heatPump.HoldRemaining_s,
                coolingActive: snapshot.heatPump.CoolingActive},
            coordination: copyFields(snapshot.coordination, COORDINATION_FIELDS)};
    }
    ...
    copyContext(value) {
        return {...copyFields(value, ['selectedWallbox', 'realOutputsEnabled', 'dataValid', 'planValid', 'grid_W', 'pv_W']),
            ...
            ehz: copyFields(value?.ehz, ['actual_W', 'target_W', 'command_W', 'active', 'owned', 'status', ...HEATER_RESERVATION_FIELDS]),
            battery: copyFields(value?.battery, BATTERY_CONTEXT_FIELDS),
            heating: copyFields(value?.heating, HEATING_CONTEXT_FIELDS),
            heatPump: copyFields(value?.heatPump, HEAT_PUMP_CONTEXT_FIELDS),
            coordination: copyFields(value?.coordination, COORDINATION_FIELDS)};
    }
    ...
    capture(id, state, previous) {
        ...
        // EHZ and battery publish Active before Status. Enrich that stop edge
        ...
        const statusDevice = relative === 'Devices.MyPV_DHW.OutputStatus' ? 'ehz'
            : relative === 'Devices.Battery.OutputStatus' ? 'battery' : null;
        if (statusDevice && this.pendingStops.has(statusDevice)) {
            this.guard(() => {
                const pending = this.pendingStops.get(statusDevice);
                this.pendingStops.delete(statusDevice);
                ...
                pending.event.context[statusDevice] = this.context(this.snapshot())[statusDevice];
            });
            return;
        }
        ...
        this.guard(() => {
            ...
            const activeDevice = relative === 'Devices.MyPV_DHW.OutputActive' ? 'ehz'
                : relative === 'Devices.Battery.OutputActive' ? 'battery' : null;
            if (activeDevice && old === true && next === false)
                this.pendingStops.set(activeDevice, {event: this.events.at(-1), at: snapshot.ts});
    ...
    takeSample(finalReason) {
        ...
        this.publish('EHZ.Summary', `${snapshot.ehz.OutputStatus || 'Status unbekannt'}; Soll ${watts(number(snapshot.ehz.target_W))}, Befehl ${watts(number(snapshot.ehz.OutputCommand_W))}, Ist ${watts(snapshot.ehz.actual_W)}; reserviert ${watts(number(snapshot.ehz.OutputReservedPower_W))}; ${snapshot.ehz.ControlReason || ''}`);
        this.publish('Battery.Summary', `${snapshot.battery.OutputStatus || 'Status unbekannt'}; Soll intern ${watts(number(snapshot.battery.target_W))}, AC-Ist intern ${watts(snapshot.battery.actual_W)} (+ Laden); GS-Befehl ${watts(number(snapshot.battery.OutputCommand_W))} (+ Entladen); reservierte Ladung ${watts(number(snapshot.battery.OutputReservedCharge_W))}; unbeobachteter Befehl ${watts(number(snapshot.battery.OutputUnobservedCommand_W))}; SoC ${snapshot.battery.soc_pct ?? '?'} %${snapshot.battery.Fault ? `; FEHLER: ${snapshot.battery.Fault}` : ''}`);
        this.publish('Heating.Summary', `${snapshot.heating.OutputStatus || snapshot.heating.Status || 'Status unbekannt'}; Soll ${watts(number(snapshot.heating.target_W))}, Ist ${watts(snapshot.heating.actual_W)}; reserviert ${watts(number(snapshot.heating.OutputReservedPower_W))}; Kuehlsperre ${snapshot.heating.CoolingBlocked ?? '?'}; manuelle Sperre ${snapshot.heating.Inhibited ?? '?'}`);
        this.publish('HeatPump.Summary', `Nur Empfehlung: ${snapshot.heatPump.RequestedMode || '?'}; gueltig ${snapshot.heatPump.AdviceValid ?? '?'}; Haltezeit ${snapshot.heatPump.HoldRemaining_s ?? '?'} s; ${snapshot.heatPump.AdviceReason || 'Noch keine Empfehlung'}`);
        this.publish('Coordination.Summary', `Feinregler ${snapshot.coordination.FineRegulator || 'unbekannt'}; ${snapshot.coordination.CoordinationStatus || 'Noch keine Koordination'}; Batterie-PV ${watts(number(snapshot.coordination.BatteryPVReserve_W))}; EHZ-PV ${watts(number(snapshot.coordination.HeaterPVAllocation_W))}; EHZ-Netzbudget ${watts(number(snapshot.coordination.HeaterCheapGridAllocation_W))}`);
    ...
    publishHistory() {
        this.publish('Events_JSON', this.serializeRing(this.events));
        this.publish('PowerTrace_JSON', this.serializeRing(this.trace));
    ...
    serializeRing(ring) {
        let json = JSON.stringify(ring);
        let bytes = Buffer.byteLength(json);
        // Keep exports within the same byte budget accepted during restart.
        // Exceptionally long UTF-8 reasons may retain fewer than 100/120 rows.
        while (bytes > MAX_RESTORE_BYTES && ring.length) {
            ring.splice(0, Math.max(1, Math.ceil(ring.length * (1 - MAX_RESTORE_BYTES / bytes))));
            json = JSON.stringify(ring);
