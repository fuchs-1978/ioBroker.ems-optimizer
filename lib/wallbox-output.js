...
class WallboxOutput {
    ...
    async update(d, mapping) {
        ...
        if (consumptionLimit.active) {
            if (!Number.isFinite(dhwActualW) || dhwActualW < 0)
                return this.stop(d, 'EHZ-Leistung fuer gemeinsames LPC-Budget fehlt/ungueltig');
            maxA = Math.min(maxA,
                Math.max(0, Number(consumptionLimit.budgetW) - dhwActualW) / (230 * phases));
        }
        ...
        for (const p of used) {
            ...
            maxA = Math.min(maxA, ownPhaseA + haLimit - house[p]);
            if (house[p] > increaseLimit) maxA = Math.min(maxA, ownPhaseA);
        }
