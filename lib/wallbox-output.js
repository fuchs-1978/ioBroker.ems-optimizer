...
class WallboxOutput {
    ...
    gate(d, mapping) {
        ...
        if (this.devices.filter(x => this.value(x.wb, 'ControlEnabled', false)
            && this.value(x.wb, 'Present', true)).length !== 1 || this.own('Devices.MyPV_DHW.ControlEnabled'))
            return 'Version 0.13: nur eine produktive Wallbox, EHZ-Steuerfreigabe muss aus sein';
        ...
        // Until the common power allocator is commissioned, curtailment pauses this single test.
        if (mapping.DP_PAR14A && ![false, 0].includes(this.state(mapping.DP_PAR14A)?.val))
            return 'Netzbetreiberbegrenzung aktiv/unklar: Einzeltest pausiert';
        if (mapping.DP_LPC_STATE && this.state(mapping.DP_LPC_STATE)?.val !== 'unlimitedAutonomous')
            return 'LPC begrenzt/unklar: Einzeltest pausiert';
        return '';
    }

    async update(d, mapping) {
        const reason = this.gate(d, mapping);
        ...
        const minimumA = Math.max(6, Number(this.value(wb, phases === 3 ? 'MinCurrent3pA' : 'MinCurrent1pA', 6)));
        ...
        const house = [1, 2, 3].map(p => this.number(this.adapter.config[`dhwHaL${p}CurrentId`]
        ...
        if ([...current, ...house, actualKW, importW, exportW, feedbackA, allow].some(v => v === null || v < 0))
        ...
        const requestedW = mandatory ? Math.max(planW, maxA * phases * 230) : Math.min(planW, nvpW);
        ...
        this.publish(wb, 'OutputStatus', `PRODUKTIV: ${d.lastA} A / ${phases} Phase(n), feste Topologie`);
    ...
    async tick() {
        ...
        try {
            ...
            this.adapter.setCompatState(`${this.adapter.namespace}.Control.Mode`, active ? 'WALLBOX_SINGLE_TEST'
