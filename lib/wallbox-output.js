...
class WallboxOutput {
    ...
    async initialize() {
        ...
        for (let wb = 0; wb < 3; wb++) {
            ...
            const d = {wb, ids, owned: false, recovering: false, wasOwned: false, wasActive: false,
                recoveredAt: 0, pending: null, lastA: 0, lastAt: 0, fault: '', valid: false};
            this.devices.push(d);
    ...
    async update(d, mapping) {
        ...
        this.publish(wb, 'OutputStatus', `PRODUKTIV: ${d.lastA} A / ${phases} Phase(n), feste Topologie${combined ? '; EHZ-Feinregelung' : ''}${recoveryStatus}`);
