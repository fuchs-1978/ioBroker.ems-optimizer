...
class WallboxOutput {
    ...
    measurementMaxAgeMs() {
        return Math.max(5,
            Number(this.adapter.config.wallboxMeasurementMaxAgeS ?? 30)) * 1000;
    }
    ...
    restartDataFresh() {
        ...
        return plan?.val === true && plan.ack === true && Number(plan.ts || 0) >= handoffSince
            && systemLastUpdate >= handoffSince && controlLastUpdate >= handoffSince;
    }
    ...
    async stop(d, reason) {
        ...
        const wasControlled = d.recovering
            || this.own(`Devices.Wallbox${d.wb}.OutputActive`) === true;
        if (wasControlled) {
    ...
    async update(d, mapping) {
        ...
        if (d.recovering && this.restartHandoffWaiting(d) && !this.restartDataFresh()) {
        ...
        const feedbackA = this.number(d.ids.feedback, deviceMaxAgeMs);
        const allow = this.number(d.ids.allow, deviceMaxAgeMs);
        ...
        const restartGraceMs = Math.max(10,
            Number(this.adapter.config.wallboxRestartHandoffGraceS ?? 30)) * 1000;
        ...
        if (!Number.isFinite(amps) || amps < minimumA) {
            if (!minimumRunActive && !restartGraceActive && !startSequenceActive) {
        ...
        if (d.recovering) {
            ...
            const settleMs = Math.max(0,
                Number(this.adapter.config.wallboxRestartHandoffSettleS ?? 10)) * 1000;
            ...
            d.lastA = feedbackA;
            d.lastAt = Date.now();
            // A restart loses the in-memory run timer. Begin a fresh minimum
            // run interval after the safe handoff instead of adopting an old
            // persisted lc timestamp that may already be expired.
        ...
        this.publish(wb, 'OutputStatus', `PRODUKTIV: ${d.lastA} A / ${phases} Phase(n)${phaseStatus}${combined ? '; EHZ-Feinregelung' : ''}${runStatus}${graceStatus}${recoveryStatus}`);
